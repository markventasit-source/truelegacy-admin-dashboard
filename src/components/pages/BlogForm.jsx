...
import { useBlogById, useCreateBlog, useUpdateBlog } from "@/store/useBlog";
...
const BlogForm = () => {
  ...
  const { data: blogData } = useBlogById(editId);
  ...
  useEffect(() => {
    const blog = unwrapContentEntity(blogData);
    if (!editId || !blog) return;
    // Only hydrate once per edit id so tab-focus refetches don't wipe drafts
    if (hydratedIdRef.current === editId) return;
    hydratedIdRef.current = editId;
    reset({
      title: blog.title || "",
      slug: blog.slug || "",
      dark_content: blog.dark_content || "",
      faded_content: blog.faded_content || "",
      image_alt: blog.image_alt || "",
      related_blogs: toRelatedBlogIds(blog.related_blogs),
      sub_sections: blog.sub_sections || [],
      faqs: blog.faqs || [],
  ...
  const onSubmit = async (data) => {
    ...
    const action = editId ? updateBlog : addBlog;
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="border border-gray-300 rounded-md p-2">
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select related blogs" />
                  </SelectTrigger>
                  <SelectContent>
                    {loadingBlogs ? (
            ...
            <div className="flex justify-end gap-4 pt-4">
              ...
              <Button
                type="submit"
                disabled={addBlog.isPending || updateBlog.isPending}
              >
                {addBlog.isPending || updateBlog.isPending
                  ? "Submitting..."
                  : "Submit"}
              </Button>
            </div>
