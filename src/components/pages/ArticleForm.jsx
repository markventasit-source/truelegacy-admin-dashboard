...
const ArticleForm = () => {
  ...
  const { data: articleData } = useArticleById(editId);
  ...
  useEffect(() => {
    const article = unwrapContentEntity(articleData);
    if (!editId || !article) return;
    if (hydratedIdRef.current === editId) return;
    hydratedIdRef.current = editId;
    reset({
      title: article.title || "",
      slug: article.slug || "",
      dark_content: article.dark_content || "",
      faded_content: article.faded_content || "",
  ...
  const onSubmit = async (data) => {
    ...
    const action = editId ? updateArticle : addArticle;
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="border border-gray-300 rounded-md p-2">
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select related articles" />
                  </SelectTrigger>
                  <SelectContent>
                    {loadingArticles ? (
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {(watch("related_blogs") || []).map((id) => {
                    ...
                    return (
                      ...
                      >
                        <span>{article?.title || "Unknown"}</span>
                        <button
                          type="button"
