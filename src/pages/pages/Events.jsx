...
const Events = () => {
  ...
  const { page, setPage, rowsPerPage, setRowsPerPage } =
    usePersistedPagination("pages:events");
  ...
  const { mutateAsync: deleteEvent, isLoading: isDeleting } =
    useDeleteEvent();
  const { mutate: updateEventMutation } = useUpdateEvent();
  const { mutate: moveMutation } = useMoveContentType("event");
  ...
  const handleSelectAll = (checked) => {
    if (checked) {
      setSelected(events?.map((p) => p._id));
    } else {
      setSelected([]);
    }
  };
  ...
  const handleOpenEdit = (id) => {
    navigate({ to: "/pages/events/edit/$id", params: { id: id } });
  };
  ...
  const handleConfirmDelete = async () => {
    try {
      if (deleteIds.length > 1) {
        await deleteEvent(deleteIds);
        toast.success("Selected events deleted successfully");
      } else {
        await deleteEvent(deleteIds[0]);
  ...
  const handlePublishToggle = (event) => {
    const newStatus =
      event.status === "published" ? "drafted" : "published";
  ...
  const getMoveSubMenu = (item) => ({
    label: "Move to",
    icon: ArrowRightLeft,
    items: ["blog", "article", "news"].map((type) => ({
  ...
  return (
    <div className="space-y-6 mt-4">
      ...
      <DeleteConfirm
        ...
        data={deleteIds.length > 1 ? "events" : "event"}
      />
    </div>
  );
};
